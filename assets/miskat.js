...
(function () {
  ...
  function shareHadis(h) {
    var url = location.origin + routeBase() + "/miskat/" + h.id;
    var title = t({ tr: "Dost Arabî", en: "Dost Arabi", pt: "Dost Arabi" }) + " — " + t(h.title);
    if (navigator.share) {
  ...
  function renderMap() {
    ...
    mapScene = window.DostHelix.mount(mapEl, {
      id: "miskat-map",
      nodes: nodes,
      // Yüz bir düğüm, Füsûs'un 27 düğümüyle aynı turns/hRatio/maxH'ı
      // paylaşıyordu -- aynı sarmal uzunluğuna ~3,8 kat daha sık paket
      // edilince düğümler üst üste biniyordu (kullanıcı bildirimi,
      // 2026-08-16). turns'ü artırmak (açısal aralığı büyütür, dikey adımı
      // etkilemez) ve maxH'ı artırmak (geniş ekranlarda daha fazla dikey
      // alan) tek başına yeterli -- helix.js'in konum motoruna dokunmadan.
      turns: 6,
      closing: false,
      hRatio: 1.45,
      maxH: 900,
